...
function chooseCoin(coins, twoWayMode) {
  ...
  cashIn.html('Comprar<br/>' + defaultCoin.display);
  cashOut.html('Vender<br/>' + defaultCoin.display);
...
function switchCoin(coin) {
  ...
  setTimeout(function () {
    return cashIn.html('Comprar<br/>' + coin.display);
  }, 100);
  setTimeout(function () {
  ...
  setTimeout(function () {
    ...
    setTimeout(function () {
      return cashOut.html('Vender<br/>' + coin.display);
    }, 100);
    setTimeout(function () {
...
function setScreen(newScreen, oldScreen) {
  ...
  if (newScreen === 'insert_bills') {
    $('.js-processing-bill').html(locale.translate('Cajero.cl | La revolución del dinero.').fetch());
...
function setCredit(fiat, crypto, lastBill, cryptoCode) {
  ...
  var inserted = lastBill ? locale.translate('Insertaste un billete de %s ').fetch(formatFiat(lastBill)) : locale.translate('Cajero.cl | La revolución del dinero.').fetch();
...
function setExchangeRate(_rates) {
  ...
  var translated = locale.translate('Nuestro precio de %s actualmente es %s').fetch(cryptoCode, rateStr);
...
function highBill(highestBill, reason) {
  var reasonText = reason === 'transactionLimit' ? 'Limite de transaccion alcanzado.' : "Estamos llegando al limite.";
  t('high-bill-header', locale.translate(reasonText).fetch());
  t('highest-bill', locale.translate('Porfavor inserta %s o menos.').fetch(formatFiat(highestBill)));
  setScreen('high_bill');
  window.setTimeout(revertScreen, 3000);
}

function minimumTx(lowestBill) {
  t('lowest-bill', locale.translate('Porfavor inserta %s o mas.').fetch(formatFiat(lowestBill)));
  setScreen('minimum_tx');
  window.setTimeout(revertScreen, 3000);
}

function readingBill(bill) {
  $('.js-processing-bill').html('Procesando ' + formatFiat(bill) + '...');
...
function sendOnly(reason, cryptoCode) {
  ...
  $('.or-circle circle').attr('r', $('#js-i18n-or').width() / 2 + 15);
  var errorMessages = {
    'transactionLimit': 'Limite de transaccion alcanzado.',
    'validatorError': 'Error en validacion.',
    'networkDown': 'Network connection error',
    'lowBalance': "Estamos sin fondos de %s",
    'blockedCustomer': ' '
  ...
  t('limit-description', locale.translate('Porfavor presiona <strong>Enviar coins</strong> para completar tu compra.').fetch());
  $('#insert-another').css({ 'display': 'none' });
  $('#limit-reached-section').css({ 'display': 'block' });

  if (reason === 'blockedCustomer') $('.blocked-customer-top').show();else $('.blocked-customer-top').hide();
}
...
function translateCoin(cryptoCode) {
  tc('total-purchased', 'total %s comprado', cryptoCode);
  tc('please-scan', 'Porfavor escanea el codigo QR para enviar tus %s.', cryptoCode);
  tc('did-send-coins', 'Ya enviaste tu %s ?', cryptoCode);
  tc('scan-address', 'Escanea tu dirección %s', cryptoCode);
  tc('coins-to-address', 'Tus %s seran enviados a:', cryptoCode);

  if (cryptoCode === 'ETH') {
    tc('authorizing-note', 'Esto deberia tomar cerca de <strong>15 segundos</strong> promedio.<br/> Podria tomar mas tiempo dependiendo del trafico en la red.');
  }
}
